...
key_type None) must not raise, and elliptic-curve / CA comparisons must be case-insensitive /
default-aware. Live-verified against real NGTS CITs (rp-digicert, rp-ztpki) separately.
"""
import unittest
...
class TestIdentityAndDrift(unittest.TestCase):
    ...
    def test_domains_diff(self):
        loc = full_ps(); loc.policy.domains = ['other.com']
        self.assertTrue(changed(loc, full_ps()))

    def test_max_valid_days_diff(self):
        loc = full_ps(); loc.policy.max_valid_days = 30
        self.assertTrue(changed(loc, full_ps()))

    def test_key_types_diff(self):
        loc = full_ps(); loc.policy.key_pair.key_types = ['RSA']
        self.assertTrue(changed(loc, full_ps()))

    def test_rsa_sizes_diff(self):
        loc = full_ps(); loc.policy.key_pair.rsa_key_sizes = [2048]
        self.assertTrue(changed(loc, full_ps()))

    def test_subject_orgs_diff(self):
        loc = full_ps(); loc.policy.subject.orgs = ['Other']
        self.assertTrue(changed(loc, full_ps()))


class TestCurveCasing(unittest.TestCase):
    def test_lowercase_curves_match(self):
        loc = full_ps(); loc.policy.key_pair.elliptic_curves = ['p256', 'p384']
        self.assertFalse(changed(loc, full_ps()))

    def test_genuine_curve_diff(self):
        loc = full_ps(); loc.policy.key_pair.elliptic_curves = ['p256']
        self.assertTrue(changed(loc, full_ps()))

    def test_default_curve_casing_match(self):
        loc = full_ps(); loc.defaults.key_pair.elliptic_curve = 'p256'
        self.assertFalse(changed(loc, full_ps()))
...
class TestCertAuthorityLandmine(unittest.TestCase):
    def test_local_default_ca_is_ignored(self):
        ...
        loc = full_ps(); loc.policy.certificate_authority = DEFAULT_CA
        self.assertFalse(changed(loc, full_ps()))

    def test_genuine_ca_diff_detected(self):
        loc = full_ps(); loc.policy.certificate_authority = 'MYCA\\Intermediate\\Template'
        rem = full_ps(); rem.policy.certificate_authority = 'OTHER\\Intermediate\\Template'
...
class TestFormerCrashPaths(unittest.TestCase):
    def test_default_key_type_none_no_crash(self):
        ...
        loc = full_ps(); loc.defaults.key_pair.key_type = None
        rem = full_ps(); rem.defaults.key_pair.key_type = None
    ...
    def test_remote_default_subject_empty_no_crash(self):
        ...
        rem = full_ps(); rem.defaults.subject = DefaultSubject()  # all None -> empty
    ...
    def test_both_default_subjects_populated_orgunits_diff(self):
        ...
        loc = full_ps(); loc.defaults.subject.org_units = ['IT']
        rem = full_ps(); rem.defaults.subject.org_units = ['Eng']
        self.assertTrue(changed(loc, rem))
...
class TestNgtsIgnoreOwnersUsers(unittest.TestCase):
    def test_ngts_skips_owners_users(self):
        ...
        loc = full_ps()
        rem = full_ps(); rem.owners = []; rem.users = []; rem.approvers = []; rem.user_access = 'rw'
